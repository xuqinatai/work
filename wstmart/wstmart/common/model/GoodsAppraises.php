...
class GoodsAppraises extends Base{
	...
	*/
	public function getById(){
		...
		$rs  = 	$this->alias('ga')
					 ->field('ga.content,ga.images,ga.shopReply,ga.replyTime,ga.createTime,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.shopId,ga.orderId,s.shopName,u.userPhoto,u.loginName,u.userTotalScore,goodsSpecNames')
		...
		foreach($rs['Rows'] as $k=>$v){
			...
			//获取用户等级
			$rrs = Db::name('user_ranks')->where('startScore','<=',$v['userTotalScore'])->where('endScore','>=',$v['userTotalScore'])->where('dataFlag=1')->field('rankId,rankName,userrankImg')->find();
